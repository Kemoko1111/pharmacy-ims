...
| A05 | Security Misconfiguration | ⚠️ | `helmet()` security headers on the API; CORS restricted to the web origin via env; secrets only in platform env vars (`.env` gitignored). **Gap:** no Content-Security-Policy on the web app yet; add a CSP meta/header before Demo Day. |
| A06 | Vulnerable & Outdated Components | ⚠️ | Current majors (NestJS 11, Prisma 6, React 18, Vite 6); `pnpm-lock.yaml` committed; pnpm 10 blocks unapproved postinstall scripts. **Gap:** no automated audit in CI — add `pnpm audit --audit-level high` job. |
...
- [ ] Add CSP header to the web app (Vercel `headers` config)
- [ ] `pnpm audit` job in CI
